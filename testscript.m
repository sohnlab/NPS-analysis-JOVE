...
ASLS_param = struct('lambda',1e9, 'p',0, 'noise_margin',2.4e-4, 'max_iter',40);
thresholds = [1e-4, 0.5e-3];
...
% ch_height = 30; % [um] (approximate)
% De_np = 2 * ch_height * w_NP / (ch_height + w_NP); % effective diameter estimate
% ASLS_param = struct('lambda',1e9, 'p',0, 'noise_margin',1.8e-4, 'max_iter',40);
% thresholds = [1e-4, 0.5e-3];
